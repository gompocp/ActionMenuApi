...
namespace ActionMenuApi.Api
{
    public static class VRCActionMenuPage
    {
        ...
        public static PedalFourAxis AddFourAxisPuppet(ActionMenuPage pageType, string text, Action<Vector2> onUpdate,Texture2D icon = null, bool locked = false, string topButtonText = "Up", 
            string rightButtonText = "Right", string downButtonText = "Down", string leftButtonText = "Left", Insertion insertion = Insertion.Post)
        {
            var pedal = new PedalFourAxis(text, icon, onUpdate, topButtonText, rightButtonText, downButtonText, leftButtonText, locked);
            AddPedalToList(pageType, pedal, insertion);
            return pedal;
        }
